...
export default async function DashboardPage() {
  ...
  const esSolicitante =
    profile.rol === "administrativo" || profile.rol === "docente" || profile.rol === "mantenimiento";
  const puedeRevisar = esSolicitante
    ? false
    : roleGrantsCapability(profile.rol, "revisar_solicitudes") ||
      (await hasCapability(user.id, "revisar_solicitudes"));
  const puedeAprobar = esSolicitante
    ? false
      ...
      <div className="dashboard-grid">
        ...
        {profile.rol === "decano" ? (
          <>
            ...
            <article className="card dashboard-tile stack" style={{ borderLeftColor: "var(--color-info)" }}>
              <h2 style={{ margin: 0 }}>Reportes</h2>
              <p className="field-hint">Consolidados por tipo de solicitud y periodo.</p>
              <Link href="/secretaria/reportes" className="btn btn--primary btn--sm" style={{ width: "fit-content" }}>
                Abrir reportes
              </Link>
            </article>
          </>
