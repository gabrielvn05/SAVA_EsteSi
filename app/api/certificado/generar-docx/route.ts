...
export async function POST(req: Request) {
  ...
  const codigoTramite = buildCodigoTramite(body.tipo, solicitante.nombres, solicitante.apellidos);
  const detalleConCodigo = { ...detalleConPersonal, codigo_tramite: codigoTramite };
  ...
  const docx = await buildOficioDocxBuffer(
    {
      ...
      fecha_fin,
      motivo,
      detalle: detalleConCodigo
    },
    destinatario
  );

  const nombreArchivo = nombreArchivoOficio(codigoTramite);
