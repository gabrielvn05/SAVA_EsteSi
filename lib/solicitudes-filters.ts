...
export function carreraFromSolicitud(row: SolicitudListRow): string {
  const d = row.detalle;
  if (d && typeof d === "object" && typeof d.carrera === "string" && d.carrera.trim()) {
    return labelCarrera(d.carrera);
  }
  return "";
}
...
export function rowMatchesSolicitudFilters(row: SolicitudListRow, f: SolicitudFiltros): boolean {
  ...
  const qCarrera = f.carrera.trim();
  if (qCarrera) {
    const carreraDetalle = row.detalle && typeof row.detalle.carrera === "string" ? row.detalle.carrera : "";
    if (carreraDetalle !== qCarrera) return false;
  }
