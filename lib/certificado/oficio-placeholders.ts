...
import { buildCodigoTramite } from "@/lib/codigo-tramite";
...
export function generarNumeroOficio(
  ...
  nombres: string,
  apellidos: string,
  codigoGuardado?: unknown
) {
  if (typeof codigoGuardado === "string" && codigoGuardado.trim()) return codigoGuardado.trim();
  return buildCodigoTramite(tipo, nombres, apellidos);
}
...
export function buildOficioReplacements(
  ...
  const numeroOficio = generarNumeroOficio(
    input.tipo,
    input.solicitante.nombres,
    input.solicitante.apellidos,
    d.codigo_tramite
  );
